...
public class MultiPeer: NSObject {
    ...
    /** Device's name */
    private (set) public var devicePeerID: MCPeerID!
    ...
    public func send(object: Any, type: UInt32, toPeers: [Peer]) {
        if isConnected {
            let data = NSKeyedArchiver.archivedData(withRootObject: object)
            
            send(data: data, type: type, toPeers: toPeers)
        }
    }
    ...
    /// After sending the data, you can use the extension for Data, `convertData()` to convert it back into data.
    public func send(object: Any, type: UInt32, toPeer: Peer) {
        self.send(object: object, type: type, toPeers: [toPeer])
    }
